...
export default {
  ...
  methods: {
    handleResize() {
      // Debounce resize to prevent constant redraws
      if (this.resizeTimeout) {
        clearTimeout(this.resizeTimeout);
      }
      
      this.resizeTimeout = setTimeout(() => {
        if (this.p5Instance && this.$refs.canvasContainer) {
          const containerRect = this.$refs.canvasContainer.getBoundingClientRect();
          const containerWidth = containerRect.width;
          const containerHeight = containerRect.height;
          
          if (containerWidth > 0 && containerHeight > 0) {
            this.p5Instance._renderer.resize(containerWidth, containerHeight);
            
            // If we have access to the sketch variables:
      ...
      }, 200); // 200ms delay to avoid frequent updates
    },
    ...
    createP5Instance() {
      const sketch = (p) => {
        ...
        p.setup = () => {
          ...
          const containerRect = this.$refs.canvasContainer.getBoundingClientRect();
          const canvas = p.createCanvas(containerRect.width, containerRect.height, p.WEBGL);
          canvas.parent(this.$refs.canvasContainer);
          
          // Create shader
          shader = p.createShader(vertexShader, fragmentShader);
          
          // Create graphics buffer for text
